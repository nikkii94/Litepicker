...
test('new DateTime', () => {
  ...
  expect(dt1 instanceof CustomDate && !isNaN(dt1.getTime())).toBe(true);

  const dt2 = new DateTime(date);
  expect(dt2 instanceof CustomDate && date.getTime() === dt2.getTime()).toBe(true);
  ...
  expect(dt3 instanceof CustomDate && date.getTime() === dt3.getTime()).toBe(false);

  const dt4 = new DateTime(`${day}/${month}/${year}`, 'DD/MM/YYYY');
  expect(dt4 instanceof CustomDate && date.getTime() === dt4.getTime()).toBe(true);
});
